...
export function GamePlayView({
  ...
  function pickRandomCard() {
    const randomCard =
      blackJackGame["cards"][Math.floor(Math.random() * 11) + 1];
  ...
  function pickExtraCard() {
    ...
    const cardStrings = [...cards, { card: card.card, symbol: card.symbol }]
  ...
  function parseOpponentCards() {
    ...
    const parsedCards = opponentCards.map((value, index) => {
      if (
        opponentCardSymbols[
          canViewAllOpponentCards || isAlice ? index : index + 1
        ]
      ) {
        return {
          card: value,
